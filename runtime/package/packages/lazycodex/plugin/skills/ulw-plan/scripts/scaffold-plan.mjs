...
// two things genuinely not guaranteed on native Windows across the LazyCodex harnesses.
//
...
// own writes to resolve under .lazycodex/ (it does not, and cannot, contain other Bash
// commands; it only guarantees the mandated generator never escapes .lazycodex). Mirrors
// the upstream plan path policy.
...
// Resolve a project-relative path and confine it under .lazycodex/ - the script's own
...
export function resolveSafeOmoPath(cwd, relPath) {
	...
	if (!/(^|[/\\])\.lazycodex([/\\]|$)/i.test(rel)) {
		throw new Error(`refused: ulw-plan may only write under .lazycodex/: ${relPath}`);
	}
...
async function assertSafeWriteParent(cwd, target) {
	...
	assertContainedPath(lazycodexRoot, parent, `refused: ulw-plan may only write under .lazycodex/: ${target}`);
	await mkdirWithoutSymlinks(parent, workspaceRoot);
	const lazycodexReal = await realpath(lazycodexRoot);
	const parentReal = await realpath(parent);
	assertContainedPath(workspaceReal, parentReal, `refused: path escapes the workspace root through symlinks: ${target}`);
	assertContainedPath(lazycodexReal, parentReal, `refused: ulw-plan may only write under .lazycodex/ through real paths: ${target}`);
}
...
status: drafting
intent: ${intent}
pending-action: write .lazycodex/plans/${slug}.md
...
- Evidence: .lazycodex/evidence/task-<N>-${slug}.<ext>
...
- [ ] 1. <title>
  ...
  QA scenarios (name the exact tool + invocation): happy + failure, Evidence .lazycodex/evidence/task-1-${slug}.<ext>
...
export async function scaffold(cwd, { slug, intent, reset = false, force = false }) {
	const draftRel = join(".lazycodex", "drafts", `${slug}.md`);
	const planRel = join(".lazycodex", "plans", `${slug}.md`);
