...
the WHOLE notepad FIRST — `cat "$NOTE"`, or read the path
...
- Repo-wide inspection, CLI smoke tests, git/history, bounded command
  output → use direct shell commands first. Pass ordinary
  commands as executable and arguments in separate argv tokens, for
  example `rg --files`; not `rg --files`,
...
`Plan: .lazycodex/plans/<slug>.md`. Do NOT auto-`git commit` unless the user
