...
## Goal
Deliver every goal in `.lazycodex/ulw-loop/goals.json` end-to-end.
...
Audit each pass, fail, block, steering change, and checkpoint in `.lazycodex/ulw-loop/ledger.jsonl`.
...
## Artifacts
- `.lazycodex/ulw-loop/brief.md`: original brief and durable constraints.
- `.lazycodex/ulw-loop/goals.json`: goals with embedded `successCriteria` per goal.
- `.lazycodex/ulw-loop/ledger.jsonl`: append-only audit trail.
- Read artifacts before resuming, steering, or checkpointing.
- After compaction or context loss, re-read brief + goals + ledger FIRST via `cat .lazycodex/ulw-loop/ledger.jsonl` (or direct paths), then `lazycodex ulw-loop status --json`. Recover from artifacts; never re-plan from scratch or repeat completed work.
- Never invent state outside `.lazycodex/ulw-loop` artifacts or `lazycodex ulw-loop status --json`.
...
ULW_LOOP_CLI=
if command -v lazycodex >/dev/null 2>&1 && lazycodex ulw-loop help >/dev/null 2>&1; then
  ULW_LOOP_CLI=lazycodex
elif [ -n "$ULW_LOOP_NODE" ]; then
  for candidate in "$HOME/.local/bin/lazycodex" "$CODEX_HOME/bin/lazycodex" "$CODEX_HOME"/plugins/cache/sisyphuslabs/lazycodex/*/components/ulw-loop/dist/cli.js; do
  ...
  if [ -n "$ULW_LOOP_CLI" ] && [ -n "$ULW_LOOP_NODE" ]; then
    lazycodex() { "$ULW_LOOP_NODE" "$ULW_LOOP_CLI" "$@"; }
  fi
fi

if [ -z "${ULW_LOOP_CLI:-}" ]; then
  /bin/mkdir -p .lazycodex/ulw-loop 2>/dev/null || mkdir -p .lazycodex/ulw-loop 2>/dev/null || true
  NOTE="${NOTE:-.lazycodex/ulw-loop/bootstrap-notepad.md}"
  printf '%s\n' "No ulw-loop-capable lazycodex executable found; PATH lazycodex may be the OpenCode CLI without the Codex ulw-loop subcommand, and cached ulw-loop CLI was not found under ${CODEX_HOME:-$HOME/.codex}." >> "$NOTE" 2>/dev/null || true
...
Run one form:
```sh
lazycodex ulw-loop create-goals --brief "<brief>" --json
lazycodex ulw-loop create-goals --brief-file <path> --json
cat <brief> | lazycodex ulw-loop create-goals --from-stdin --json
```
...
`lazycodex ulw-loop create-goals --session-id <new-id> ...`; use `--force`
...
### 3. Inspect state
Run `lazycodex ulw-loop status --json`.
...
### Acquire Next Goal
1. Run `lazycodex ulw-loop complete-goals --json` and read the handoff, including criteria.
...
4. If retrying failed work, run `lazycodex ulw-loop complete-goals --retry-failed --json`.
...
8. RECORD exactly one result:
   - PASS: `lazycodex ulw-loop record-evidence --goal-id <id> --criterion-id <id> --status pass --evidence "<observable> | <cleanup receipt>" --json`
   - FAIL: `lazycodex ulw-loop record-evidence --goal-id <id> --criterion-id <id> --status fail --evidence "<observable> | <cleanup receipt>" --notes "<diagnosis>" --json`
   - BLOCKED: `lazycodex ulw-loop record-evidence --goal-id <id> --criterion-id <id> --status blocked --evidence "<observable>" --notes "<safety/blocker/leftover-state>" --json`
...
3. Run `lazycodex ulw-loop checkpoint --goal-id <id> --status complete --evidence "<criteria evidence summary>" --codex-goal-json <snapshot> --json`.
...
5. If review remains blocked, run `lazycodex ulw-loop record-review-blockers --goal-id <id> --title "<...>" --objective "<...>" --evidence "<review findings>" --codex-goal-json <snapshot> --json`.
6. If clean, checkpoint final completion:
```sh
lazycodex ulw-loop checkpoint --goal-id <id> --status complete --evidence "<e2e evidence + manual QA notes>" --codex-goal-json <snapshot> --quality-gate-json <json-or-path> --json
```
`--quality-gate-json` shape:
```json
{
  "codeReview":{"by":"lazycodex-code-reviewer","recommendation":"APPROVE","codeQualityStatus":"CLEAR","reportPath":"packages/lazycodex/plugin/components/ulw-loop/test/fixtures/artifacts/code-review.md","evidence":"Diff review passed.","blockers":[]},
  "manualQa":{"by":"lazycodex-qa-executor","status":"passed","evidence":"CLI and data surfaces passed.","surfaceEvidence":[{"id":"surface-cli-pass","criterionRef":"C1","surface":"cli","invocation":"lazycodex ulw-loop checkpoint --quality-gate-json sample-quality-gate.json --json","verdict":"passed","artifactRefs":["artifact-cli-pass"]},{"id":"surface-data-pass","criterionRef":"C2","surface":"data","invocation":"diff -u before-ledger.json after-ledger.json","verdict":"passed","artifactRefs":["artifact-data-diff"]}],"adversarialCases":[{"id":"adv-malformed-input","criterionRef":"C3","scenario":"malformed gate input omits manual QA evidence","expectedBehavior":"validator rejects ULW_LOOP_QUALITY_GATE_INVALID","verdict":"passed","artifactRefs":["artifact-cli-reject"]}],"artifactRefs":[{"id":"artifact-cli-pass","kind":"cli-transcript","description":"CLI pass artifact.","path":"packages/lazycodex/plugin/components/ulw-loop/test/fixtures/artifacts/cli-pass.txt"},{"id":"artifact-cli-reject","kind":"log","description":"Reject log artifact.","path":"packages/lazycodex/plugin/components/ulw-loop/test/fixtures/artifacts/rejection.txt"},{"id":"artifact-data-diff","kind":"data-diff","description":"Data diff artifact.","path":"packages/lazycodex/plugin/components/ulw-loop/test/fixtures/artifacts/data-diff.txt"}]},
  "gateReview":{"by":"lazycodex-gate-reviewer","recommendation":"APPROVE","reportPath":"packages/lazycodex/plugin/components/ulw-loop/test/fixtures/artifacts/gate-review.md","evidence":"Gate review passed.","blockers":[]},
  "iteration":{"fullRerun":true,"status":"passed","rerunCommands":["bunx vitest run packages/lazycodex/plugin/components/ulw-loop/test/quality-gate-doc.test.ts"],"evidence":"Focused rerun passed."},
...
Command form: `lazycodex ulw-loop steer --kind <kind> [<kind-specific-fields>] --evidence "<...>" --rationale "<...>" --json`.
Structured prompt directives accepted: `LAZYCODEX_ULW_LOOP_STEER: { ... }`, `lazycodex.ulw-loop.steer: {...}`, `lazycodex ulw-loop steer: {...}`.
...
6. Treat `.lazycodex/ulw-loop/ledger.jsonl` as the durable audit trail; checkpoint after every success or failure.
