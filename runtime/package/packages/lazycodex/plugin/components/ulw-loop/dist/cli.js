...
var ULW_LOOP_DIR = ".lazycodex/ulw-loop";
...
var ULW_LOOP_HELP = `Usage:
  lazycodex ulw-loop create-goals --brief "..." [--brief-file <path>] [--from-stdin] [--codex-goal-mode aggregate|per_story] [--force] [--json]
  lazycodex ulw-loop status [--json]
  lazycodex ulw-loop complete-goals [--retry-failed] [--json]
  lazycodex ulw-loop criteria --goal-id <id> [--json]
  lazycodex ulw-loop record-evidence --goal-id <id> --criterion-id <id> --status pass|fail|blocked --evidence "..." [--notes "..."] [--json]
  lazycodex ulw-loop checkpoint --goal-id <id> --status complete|failed|blocked --evidence "..." --codex-goal-json <...> [--quality-gate-json <...>] [--json]
  lazycodex ulw-loop steer --kind <kind> ... --evidence "..." --rationale "..." [--json]
  lazycodex ulw-loop add-goal --title "..." --objective "..." [--json]
  lazycodex ulw-loop record-review-blockers --goal-id <id> --title "..." --objective "..." --evidence "..." --codex-goal-json <...> [--json]

All subcommands accept [--session-id <id>] to isolate state under .lazycodex/ulw-loop/<id>/; without it, Codex session env is used when present.`;
...
var SESSION_ENV_KEYS = ["LAZYCODEX_ULW_LOOP_SESSION_ID", "CODEX_SESSION_ID", "CODEX_THREAD_ID"];
...
function buildCompletedLegacyGoalRemediation(goal) {
  return [
    ...
    `Record a non-terminal blocker with: lazycodex ulw-loop checkpoint --goal-id ${goal.id} --status blocked --evidence "<completed legacy Codex goal blocks create_goal in this thread>" --codex-goal-json "<different completed get_goal JSON or path>".`,
    "Then continue only from a Codex goal context with no active/completed conflicting goal, in the same repo/worktree, and create the intended goal there."
  ].join(" ");
}
function buildTaskScopedAggregateReconciliationHint(goal, final) {
  if (final) {
    return ` Final task-scoped aggregate reconciliation requires the checkpoint goal to be the active in-progress final LazyCodex goal and the completed get_goal objective to map to the ulw-loop brief or artifact. ${buildCompletedLegacyGoalRemediation(goal)}`;
  }
  return ` Completed task-scoped aggregate reconciliation requires the checkpoint goal to be the active in-progress LazyCodex goal, evidence that names that active LazyCodex goal id, names .lazycodex/ulw-loop/goals.json or ledger.jsonl, includes completed implementation plus validation/review evidence, and a get_goal objective that maps to the ulw-loop brief/artifact. ${buildCompletedLegacyGoalRemediation(goal)}`;
}
...
async function readUlwLoopPlan(repoRoot, scope) {
  ...
  try {
    ...
    if (!hasCode(error, "ENOENT"))
      throw error;
    throw new UlwLoopError(`No ulw-loop plan found at ${repoRelative(path, repoRoot)}. Run \`lazycodex ulw-loop create-goals ...\` first.`, "ULW_LOOP_PLAN_MISSING", { cause: error });
  }
...
var STEERING_KIND_HELP = [
  ...
  'Example: lazycodex ulw-loop steer --kind annotate_ledger --evidence "observed behavior" --rationale "why this changes the plan" --json'
].join(`
`);
...
function checkpointLines(plan, mode) {
  ...
  if (mode === "per_story")
    return [failureLine];
  return [
    "- Checkpoint this LazyCodex story with a fresh get_goal snapshot whose objective matches the aggregate payload.",
    failureLine
  ];
}
...
function finalSection(plan, goal, isFinal, aggregate) {
  ...
  const blockerCommand = `lazycodex ulw-loop record-review-blockers${option} --goal-id ${goal.id} --title "Resolve final code-review blockers" --objective "<blocker-resolution objective>" --evidence "<review findings>" --codex-goal-json "<active get_goal JSON or path>"`;
  const checkpointCommand = `lazycodex ulw-loop checkpoint${option} --goal-id ${goal.id} --status complete --evidence "<targeted verification/manualQa/gateReview evidence>" --codex-goal-json "<fresh complete get_goal JSON or path>" --quality-gate-json "<quality gate JSON or path>"`;
  return joinLines([
...
function sessionOption(plan) {
  const prefix = ".lazycodex/ulw-loop/";
...
function completedPlanExistsError(scope) {
  return new UlwLoopError([
    ...
    "Start a new run with `lazycodex ulw-loop create-goals --session-id <new-id> ...` to isolate fresh state.",
...
function parseUlwLoopSteeringDirective(text3) {
  const match = /(?:^|\s)(?:LAZYCODEX_ULW_LOOP_STEER|lazycodex ulw-loop steer):\s*([\s\S]+)$/u.exec(text3);
...
function hasSteeringDirectiveMarker(prompt) {
  return /(?:^|\s)(?:LAZYCODEX_ULW_LOOP_STEER|lazycodex ulw-loop steer):/u.test(prompt);
}
...
var TOP_LEVEL_HELP = `Usage:
  lazycodex ulw-loop <subcommand> [args]
  lazycodex hook user-prompt-submit [--with-ultrawork]  (Codex UserPromptSubmit hook)
  lazycodex help | --help | -h                          (this message)

Run \`lazycodex ulw-loop help\` for ulw-loop subcommands.
`;
async function main() {
  ...
  if (command === "hook") {
    ...
    process.stderr.write(`[lazycodex] unknown hook subcommand: ${sub ?? "(none)"}
`);
    return 1;
  }
  if (isUlwLoopSubcommand(command))
    return ulwLoopCommand(argv);
  process.stderr.write(`[lazycodex] unknown command: ${command}
${TOP_LEVEL_HELP}`);
  return 1;
}
main().then((code) => {
  ...
  process.stderr.write(`[lazycodex] ${error instanceof Error ? error.message : String(error)}
`);
  process.exit(1);
});
